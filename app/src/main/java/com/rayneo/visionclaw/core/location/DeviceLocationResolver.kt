...
class DeviceLocationResolver(context: Context) {
    ...
    fun peekCached(maxAgeMs: Long = DEFAULT_MAX_AGE_MS): DeviceLocationContext? {
        val snapshot = cached ?: return null
        val ageMs = System.currentTimeMillis() - snapshot.timestampMs
        return snapshot.takeIf { ageMs <= maxAgeMs }
    }
    ...
    fun resolveBlocking(
        ...
        resolve(maxAgeMs = maxAgeMs, timeoutMs = timeoutMs)
    }
    ...
    suspend fun resolve(
        ...
        peekCached(maxAgeMs)?.let { return it }
        ...
        val preferredCurrent = bestCurrent?.takeIf { isUsable(it) }
        if (preferredCurrent != null) {
            return remember(preferredCurrent)
        }

        requestWifiTriangulatedLocation(considerIp = false)?.let { triangulated ->
            return remember(triangulated)
        }

        bestLastKnown?.takeIf { isUsable(it) }?.let { return remember(it) }
