...
class OpenClawTool(
    ...
    override suspend fun execute(args: Map<String, String>): Result<String> {
        ...
        return when (val result = openClawClient.sendMessage(query, context, imageBase64)) {
            is OpenClawClient.ClawResult.Success -> {
                ...
                // Defense-in-depth: flag hallucinated media domains at source.
                // Public builds must not rewrite those guesses to a
                // maintainer-owned relay.
     ...
     *
     * Possible relay-domain hallucinations are logged via [rewriteAllUrlsInText]
     * once the response comes back — that's a different failure mode.
     */
