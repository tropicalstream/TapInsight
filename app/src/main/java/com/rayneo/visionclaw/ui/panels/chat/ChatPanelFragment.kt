...
class ChatPanelFragment : Fragment(), TrackpadPanel {
    ...
    private fun renderHeartbeat(animateHide: Boolean = false) {
        ...
        val prefix = "\u2764\uFE0F "
        hudHeartbeatText.isSelected = false          // stop current marquee cycle
        hudHeartbeatText.text = "$prefix$effectiveMessage"
        hudHeartbeatText.visibility = View.VISIBLE
        hudHeartbeatText.animate().cancel()
        hudHeartbeatText.alpha = 1f
        hudHeartbeatText.post { hudHeartbeatText.isSelected = true }  // restart marquee on next frame
    }
     ...
     * when used as a transient override. Pass 0 for the persistent ticker.
     * Pass null or blank with displayMs > 0 to clear the transient message
     * and fall back to the persistent ticker. Pass null or blank with 0 to
     * clear the persistent ticker entirely.
     */
