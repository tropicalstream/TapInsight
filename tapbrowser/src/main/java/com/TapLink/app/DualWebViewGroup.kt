    ...
    fun dispatchMaskOverlayTouch(screenX: Float, screenY: Float) {
        ...
        if (::maskVisualizerView.isInitialized && maskVisualizerView.visibility == View.VISIBLE) {
            ...
            if (screenX >= vizLoc[0] &&
                ...
                val wrapped = maskVisualizerView.cycleThemeOrWrap()
                if (wrapped) hideVisualizer() else updateVisualizerButtonColor()
                return
            }
        }
...
""".trimIndent()
                ...
                override fun onFftDataCapture(v: Visualizer?, fft: ByteArray?, samplingRate: Int) {
                    ...
                    for (i in 0 until buckets) {
                        ...
                        if (idx * 2 + 1 < fft.size) {
                            ...
                            val mag = kotlin.math.sqrt(re * re + im * im) / 128f  // normalize to ~0..1
                            fftMagnitudes[i] = mag.coerceIn(0f, 1f)
                        }
                    }
                }
    ...
    private fun updateVisualizerButtonColor() {
        ...
        val themeColor = if (!isVisualizerVisible) {
            ...
            when (maskVisualizerView.currentTheme) {
                maskVisualizerView.THEME_NEON_BARS   -> Color.parseColor("#FF00FF")
    ...
    inner class AudioVisualizerView(context: Context) : View(context) {
        ...
        var currentTheme = THEME_NEON_BARS
            private set
        ...
        /** Advance theme; returns true if we wrapped past the last theme (caller should hide). */
        fun cycleThemeOrWrap(): Boolean {
            val next = currentTheme + 1
            if (next >= THEME_COUNT) {
                currentTheme = THEME_NEON_BARS   // reset for next show
                return true                       // signal to hide
            }
            currentTheme = next
            updateVisualizerButtonColor()
            invalidate()
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            // Smooth interpolation toward targets (responsive but not jarring)
            for (i in 0 until barCount) {
                val diff = targetHeights[i] - barHeights[i]
                velocities[i] = velocities[i] * 0.6f + diff * 0.4f
                barHeights[i] = (barHeights[i] + velocities[i]).coerceIn(0.0f, 1f)
            }

            when (currentTheme) {
                THEME_NEON_BARS -> drawNeonBars(canvas, w, h)
        ...
        private fun drawNeonBars(canvas: Canvas, w: Float, h: Float) {
            val gap = 2f
            ...
            for (i in 0 until barCount) {
                ...
                paint.alpha = (barHeights[i] * 220).toInt().coerceIn(80, 220)
                canvas.drawRoundRect(x, h - barH, x + barWidth, h, 2f, 2f, paint)
                // Glow effect: faint wider bar behind
                paint.alpha = (barHeights[i] * 60).toInt().coerceIn(20, 60)
                canvas.drawRoundRect(x - 1, h - barH - 2, x + barWidth + 1, h, 3f, 3f, paint)
            }
        }
