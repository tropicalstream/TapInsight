...
""".trimIndent()
    ...
    private fun setupMaskOverlayUI() {
        ...
        // `isVisualizerVisible` guard) reference the lateinit field —
        // skipping the init would crash those paths. It is constructed
        // here, kept invisible (View.GONE), and intentionally NOT added
        // to the maskOverlay hierarchy. `showVisualizer` is unreachable
        // (no toggle button is ever wired into the layout), so the view
        // never animates and the audio-capture pipeline never starts.
        maskVisualizerView = AudioVisualizerView(context).apply {
            visibility = View.GONE
            alpha = 0.6f
        }
        // (no maskOverlay.addView for maskVisualizerView)
