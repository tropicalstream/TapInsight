...
> **Version 1.1.0**
>
> **Experimental AR software — use thoughtfully.** Maps and location features remain experimental. Follow local laws, do not rely on location services for emergencies, and remember that local data stored on the glasses may be lost if the device is lost or reset.
...
The companion app runs over WiFi at `http://<glasses-ip>:19110` (replace `<glasses-ip>` with the address shown in Settings → WiFi on the glasses). From there you can configure everything: API keys, AI model, OAuth, TapRadio stations, and more.
